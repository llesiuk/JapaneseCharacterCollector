...
public class ViewCharActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (extra.contains("kanji")) {
			progressBar.setMax(getResources().getInteger(R.integer.kanji_quantity));
		} else {
			progressBar.setMax(getResources().getInteger(R.integer.kana_quantity));
		}
	}	
	...
	protected void onResume() {
		...
		if (extra.contains("hiragana")) {
			...
			character.setImageResource(getResources().getIdentifier(extra.substring(0, 1) + KanaSyllabary.syllabaryHiragana.get(number).getCode(), "drawable", "com.jpchar"));
		} else if (extra.contains("katakana")) {
			name.setText(KanaSyllabary.syllabaryKatakana.get(number).getName());		
			character.setImageResource(getResources().getIdentifier(extra.substring(0, 1) + KanaSyllabary.syllabaryKatakana.get(number).getCode(), "drawable", "com.jpchar"));
		} else {
			name.setText(KanaSyllabary.kanji.get(number).getName());		
			character.setImageResource(getResources().getIdentifier(KanaSyllabary.kanji.get(number).getKanjiCode(), "drawable", "com.jpchar"));
		}
	}
